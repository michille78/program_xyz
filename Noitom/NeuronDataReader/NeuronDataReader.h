...
// Header format of compressed Opt data
typedef struct _OptDataHeader
{
    UINT16       Token1;           // Package start token: 0xDDFF
    DATA_VER     DataVersion;      // Version of community dat°É  a format. e.g.: 1.0.0.2
    UINT32       DataCount;        // Values count
    BOOL         WithDisp;         // With/out dispement
    BOOL         WithReference;    // With/out reference bone data at first
    UINT32       AvatarIndex;      // Avatar index
...
*      Connector reference of TCP/IP client or UDP service as identity.
*	header
*      A BvhDataHeader type pointer, to output the Opt data format information.
*	data
*		Float type array pointer, to output binary data.
*Remarks
*      The related information of the data stream can be obtained from OptDataHeader.
*/
typedef void(CALLBACK *OptDataReceived)(void* customedObj, SOCKET_REF sender, OptDataHeader* header, float* data);
