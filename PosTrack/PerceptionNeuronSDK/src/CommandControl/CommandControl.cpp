...
CCommandControl::CCommandControl(ISerialPort* communicaor)
{
	ASSERT(communicaor);
...
BOOL CCommandControl::SensorWakeup()
{
...
BOOL CCommandControl::StartCapture()
{
	ASSERT(cmdParser.Comm);
	if (!cmdParser.Comm->IsOpen())
	{
...
// 休眠
BOOL CCommandControl::SensorSleep()
{
	ASSERT(cmdParser.Comm);
...
// 关闭子节点
BOOL CCommandControl::SensorPowerOff()
{
	ASSERT(cmdParser.Comm);
...
// 查询主节点版本号
BOOL CCommandControl::QueryReceiverNodeVersion(char* buff, int buffLen)
{
	ASSERT(cmdParser.Comm);
...
// 修改通讯频率
BOOL CCommandControl::SetFrequentChannel(int freq)
{
	ASSERT(cmdParser.Comm);
...
// 进入数据模式
BOOL CCommandControl::SensorToDataMode()
{
    ASSERT(cmdParser.Comm);
    memset(msg, '\0', sizeof(msg));
...
// 查询通讯频率
BOOL CCommandControl::QueryFrequentChannel(int& result)
{
	ASSERT(cmdParser.Comm);
...
char* CCommandControl::GetMessage()
{
	return msg;
}
