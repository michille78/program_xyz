...
class CSerialPort : public CObject, public InterfaceSerialPort
{
public:
//Enums
  enum FlowControl
...
// ¼Ì³ÐµÄº¯Êý
  virtual int Read(BYTE* buff, int bytesToRead)
  {
      return Read((void*)buff, bytesToRead);
  }
  virtual int Write(BYTE* buff, int bytesToWrite)
  {
      return Write((void*)buff, bytesToWrite);
  }
...
//General Methods
  void Open(int nPort = 3, DWORD dwBaud = 921600, Parity parity = NoParity, BYTE DataBits = 8, 
  ...
  BOOL IsOpen()  { return m_hComm != INVALID_HANDLE_VALUE; };
#ifdef _DEBUG
