...
void CHybidTrack::UpdateVisionData(int OtherMarkersN, float* pOtherMarkers, float fLatency, unsigned int Timecode, unsigned int TimecodeSubframe, double fTimestamp)
{
	...
	// m_mappintVisual_k 可能大于视觉数据的最大值;
	// m_mappingInertial_k 可能大于惯性数据的最大值
	// 因此调用位移补偿函数的时候，需要进行检查，不能把最新的数据直接导入
...
// 调用 MATLAB 生成的 GetINSCompensateFromVNS 进行位移补偿
void CHybidTrack::GetDisplacementCompensate()
{
	...
	if (!rtIsNaN(m_fLatency_StartTwo))
	{
		if (CalculateOrder[0].CalEndVN == 0)  // 第一次补偿
		{
			...
			int newIneritalN = IneritalNew - IneritalN;  // 新增的惯性个数
			int inertialCompensate_k;	// 最新可用进行补偿计算的 惯性数据序号
			int visual_k;			// inertialCompensate_k 指向的视觉序号
			int CalEndVN_new ;		//  结果：新的视觉最新可用数据序号 
			for (inertialCompensate_k = IneritalNew; inertialCompensate_k > IneritalN; inertialCompensate_k--)
			{
				...
				if (visual_k<= visualN) // 惯性对应的时间 比 实际的　OK
				{
					if (M_otherMakers[visual_k - 1].inertial_k <= inertialCompensate_k) // 视觉对应的惯性也已经更新
					{
			...
			// 得到可进行补偿的惯性数据序号 inertialCompensate_k
			
			if (CalEndVN_new>CalculateOrder[0].CalEndVN && CalEndVN_new>1)  // 有数据更新
			{
				// 更新到 第 compensated_VN_new 个（从1开始）
				/// CalculateOrder 中记录的是个数（从1开始），不是序号
				
				CalculateOrder[0].CalStartVN = CalculateOrder[0].CalEndVN + 1; // 与上次保持连续
				CalculateOrder[0].CalEndVN = CalEndVN_new;	// 得到 最新的视觉数据
...
void CHybidTrack::UpdateInertialData(QUATERNION_t HipQ, Point3D_t HipP, QUATERNION_t HeadQ, Point3D_t HeadP, Vector3_t FaceDirection)
{
	...
	//  计算 m_mappintVisual_k， 从当前最新视觉的时间，根据频率细调
	int m_mappintVisual_k;
	if (rtIsNaN(m_fLatency_StartTwo))
	{
		if (m_OffLineRead)
			m_mappintVisual_k = 1; // 离线模式
		else
			m_mappintVisual_k = 0;
		if (IsBothStart == 2)
		{
			m_mappintVisual_k = 1; // 不应该出现的情况，由于线程的不同步导致
			// 清空以前的惯性数据
			InertialData.clear();
			SetM_InertialData_Empty();
		}			
